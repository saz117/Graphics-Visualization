...
void GLWidget::initializeGL() {
    ...
    glEnable(GL_CULL_FACE); //Optativa Transparencias (comentar para lanzar)
    glEnable(GL_RGBA);
    glEnable(GL_DOUBLE);
    //glDisable(GL_CULL_FACE); //Optativa Transparencias (añadir para lanzar)

    //Enable blending
    //glEnable(GL_BLEND); //Optativa Transparencias (añadir para lanzar)
    //glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA); //Optativa Transparencias (añadir para lanzar)
...
void GLWidget::initShadersGPU(){
    ...
    initShader("://resources/vGouraudShader.glsl", "://resources/fGouraudShader.glsl", 1);
    ...
    initShader("://resources/vPhongTex.glsl", "://resources/fPhongTex.glsl", 4);
}
 ...
 */
void GLWidget::initShader(const char* vShaderFile, const char* fShaderFile, int shader_id){
    ...
    idShaders[shader_id] = program; //asignamos el id del shader que pasamos, a nuestra lista de IDs para usarlo en los Activa Shader
}
...
void GLWidget::activaToonShader() {
    ...
    program = idShaders[3]; //id correspondiente a Shader
    updateShader();
}
...
void GLWidget::activaPhongShader() {
    ...
    program = idShaders[2]; //id correspondiente a Shader
    updateShader();
}
...
void GLWidget::activaGouraudShader() {
    ...
    program = idShaders[1]; //id correspondiente a Shader
    updateShader();
...
void GLWidget::activaPhongTex() {
    ...
    program = idShaders[4]; //id correspondiente a Shader PhongTex
    updateShader();
}
...
void GLWidget::setTextureFile(const QString &file)
{
    ...
    //Suponemos todos los objetos con la misma textura:
