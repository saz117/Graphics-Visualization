...
void main()
{
    ...
    for (int i=0; i < numl; i++){
        ...
        if(conjunto[i].lightType == 1) { //si es luz direccional
            ...
            L = normalize(conjunto[i].origen * -1);
    ...
    //POSIBLES SALIDAS:
    color = 0.2*color + 0.8*texture2D(texMap, v_texcoord);
    //color = texture(texMap, v_texcoord);
    //color = vec4(texture(texMap, v_texcoord).rgb, 1.0f);
}
