...
shared_ptr<Material> Mapping::mapeigMaterial(int i, ColorMapStatic::ColorMapType tCM, double valorMonReal) {
    ...
    int idx = int(255*((valorMonReal-getPropLimits(0))/(setup->propLimits[i].second-setup->propLimits[i].first)));

    //return make_shared<Metal>(cm->getColor(idx));
    return 0; //lo cambie y puse 0 porque el.h siempre retorna 0?
}
