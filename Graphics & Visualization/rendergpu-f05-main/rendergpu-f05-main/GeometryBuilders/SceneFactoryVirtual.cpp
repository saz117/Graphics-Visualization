...
shared_ptr<Scene> SceneFactoryVirtual::createScene(QString filename, shared_ptr<Camera> camera) {
...
shared_ptr<Scene> SceneFactoryVirtual::createScene(shared_ptr<Camera> camera) {
    auto scene= make_shared<Scene>();
    scene->setCamera(camera);
    ...
    shared_ptr<DataAnimation> da = make_shared<DataAnimation>(scene);
    da->config(map);
    return scene;
}
