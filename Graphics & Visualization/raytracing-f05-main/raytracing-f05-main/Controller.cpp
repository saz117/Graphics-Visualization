...
Controller::Controller(QString dataFileName, QString configMapping, QString configVis)
{
        ...
        //set de las luces en scene
        scene->setLights(visSetup->luces, visSetup->globalLight);

        //Crea la camara segun los datos del fichero
        scene->setCamera(visSetup->getCamera());
        scene->setDimensions(vec3(mapping->Vxmin,mapping->Vymin,mapping->Vzmin),vec3(mapping->Vxmax,mapping->Vymax,mapping->Vzmax));

        //inicialitzem el ground
        if(mapping->plaBaseNotNull==true){
